...
vroom::vroom("./output/A1_glcp_filtered_add_sens_slope_new.csv") %>% 
  ...
  write.table(., file = paste0("./output/A2_glcp_filtered_significant_sens_slopes_for_modeling_new.csv"),       
              append = T,
              row.names = F,
              col.names = !file.exists("./output/A2_glcp_filtered_significant_sens_slopes_for_modeling_new.csv"))



d <- vroom::vroom("./output/A2_glcp_filtered_significant_sens_slopes_for_modeling_new.csv") %>%
  select(-lake_type) %>%
  readr::write_csv("./output/A2_glcp_filtered_significant_sens_slopes_for_modeling_22Mar23.csv")


india <- d %>% filter(country == "India")%>%
  readr::write_csv("./output/A2_glcp_filtered_significant_sens_slopes_for_modeling_22Mar23_JUST_INDIA.csv")
