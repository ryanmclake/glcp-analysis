...
slope_data <- vroom::vroom("./output/slopes/hylak_id_kendall_sens.csv", delim = " ", col_names = T) %>%
  mutate_all(funs(as.numeric(.))) %>%
  filter(!(hylak_id %% 1)) %>%
  na.omit(.) %>%
  #filter(kendall_tau < 1.000000000000) %>%
...
pop_hexes_avg <- area_hexes %>%
  st_drop_geometry() %>%
  group_by(index) %>%
  summarise(pop_diff = median(pop_diff, na.rm = TRUE)) %>%
  right_join(grid, by="index") %>%
  st_sf()
...
lake_area_change <-
  ...
  scale_fill_gradient2(midpoint=0, low="tan1", mid="white",
                       ...
                       name = "**Δ Lake Surface Area** <br>z-scored km2/yr") +
...
ggsave(lake_area_change, path = ".",
       filename = "./output/figures/sens_lake_area_global_plot.jpg",
