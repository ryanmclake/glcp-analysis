cluster.reg <-function(Y,X,loop=100){
  X<-data.frame(X)
  Y<-data.frame(Y)
  
  if(ncol(X) > 4)stop(paste("too many covariates"))
  ...
  #rownames(parm)<-paste("cluster ",sort(unique(cluster[1])),sep='') # what is going on here?
