...
slope_data_shrink <- c(unique(slope_data_shrink$hylak_id))
  ...
  system.time(read_csv_arrow(
    ...
    timestamp_parsers = NULL) %>% rename(year = f0,
      ...
      mutate(precip_coeff = summary(mod)$coefficients[2],
             ...
             temp_coeff = summary(mod)$coefficients[3],
             temp_p = summary(mod)$coefficients[3,4],
             pop_coeff = summary(mod)$coefficients[4],
             pop_p = summary(mod)$coefficients[4,4],
             res_std = summary(mod)$sigma) %>%
      select(-mod) %>%
      collect() %>%
      write.table(., file = paste0("./output/hybas_id_lm_shrink_coefficients.csv"),
                  append = T,
                  row.names = F,
                  col.names = !file.exists("./output/hybas_id_lm_shrink_coefficients.csv")))
}
